...
export async function processPostData(props, from) {
  ...
  if (!props?.post?.blockMap) {
    ...
    // 仅当 id 看起来像 Notion pageId 时才尝试抓取 blockMap，避免 EmptyData stub 等无效 id 造成 parsePageId 抛错
    if (typeof postId === 'string' && postId.length >= 32) {
      try {
        props.post.blockMap = await getPostBlocks(postId, from)
      } catch (e) {
        console.warn('[processPostData] getPostBlocks failed', e?.message)
