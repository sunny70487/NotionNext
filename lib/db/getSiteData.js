    ...
    allPages: [
      {
        ...
        publishDate: new Date('2024-11-13').getTime(),
        lastEditedDate: new Date('2024-11-13'),
        lastEditedDay: '2024-11-13',
        pageIcon: '',
...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  if (rawMetadata && !rawMetadata.type && rawMetadata.value) {
  ...
  let collection = Object.values(pageRecordMap.collection)[0]?.value || {}
  if (collection && !collection.schema && collection.value) {
  ...
  if ((!collectionQuery || Object.keys(collectionQuery).length === 0) && collectionId && viewIds?.length > 0) {
    try {
      const resp = await fetch('https://www.notion.so/api/v3/queryCollection', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          collection: { id: collectionId, spaceId: rawMetadata?.space_id },
          collectionView: { id: viewIds[0], spaceId: rawMetadata?.space_id },
          loader: {
            type: 'reducer',
            reducers: {
      ...
      if (queryResult?.result?.reducerResults?.collection_group_results?.blockIds) {
        collectionQuery = {
          [collectionId]: {
            [viewIds[0]]: {
              collection_group_results: queryResult.result.reducerResults.collection_group_results
            }
          }
        }
      }
    } catch (e) {
      console.warn('[queryCollection fallback failed]', e.message)
    }
  }
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    let value = block[id]?.value
    if (value && !value.type && value.value) value = value.value
    if (!value) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    let value = block[id]?.value || fetchedBlocks[id]?.value
    if (value && !value.type && value.value) value = value.value
    const properties =
