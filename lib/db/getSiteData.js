...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  // Debug: log block keys and rawMetadata type
  const blockKeys = Object.keys(block).slice(0, 5)
  const blockEntry = block[pageId]
  console.log(`[DEBUG] pageId="${pageId}", blockKeys=${JSON.stringify(blockKeys)}, rawMetadata?.type="${rawMetadata?.type}", hasCollection=${!!pageRecordMap.collection}`)
  console.log(`[DEBUG2] blockEntry keys=${JSON.stringify(Object.keys(blockEntry || {}))}, value keys=${JSON.stringify(Object.keys(blockEntry?.value || {})).slice(0, 200)}`)
