...
import { adapterNotionBlockMap } from '@/lib/utils/notionBlockAdapter'
...
export async function getPage(id, from = null, slice) {
  return await getOrSetDataWithCache(
    ...
    async (id, slice) => {
      ...
      if (pageBlock) {
        pageBlock = adapterNotionBlockMap(pageBlock)
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    console.log('[API<<--响应]')
    const rawBlocks = pageChunk?.recordMap?.block || {}
    const normalizedBlocks = {}
    for (const [bid, block] of Object.entries(rawBlocks)) {
      const val = block?.value?.value?.id && block?.value?.role
        ? block.value.value
        : block?.value?.id && block?.role !== undefined
          ? block.value
          : block?.value?.id
            ? block.value
            : block?.value ?? block
      normalizedBlocks[bid] = { value: val }
    }
    fetchedBlocks = Object.assign({}, fetchedBlocks, normalizedBlocks)
  }
